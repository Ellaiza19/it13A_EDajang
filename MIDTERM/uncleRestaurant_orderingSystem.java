...
public class uncleRestaurant_orderingSystem {
   
   
    // Scanner to receive user input
    static Scanner scanner = new Scanner(System.in);
    // DecimalFormat to format price outputs to 2 decimal places
    static DecimalFormat df = new DecimalFormat("#.00");
    // File where usernames and passwords are stored
    ...
    public static void main(String[] args) {
        while (true) { // loop back to main menu if login fails
            ...
            if (choice == 1) {
                ...
                loggedIn = login(); // prompt user to login after account creation
                if (loggedIn) {
    ...
    static void createAccount() {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(USER_FILE, true))) {
            writer.write(username + "," + encrypted); // write to file
    ...
    static void menu() {
        ...
        double[] prices = {120.0, 150.0, 70.0, 100.0, 150.0, 50.0, 250.0}; // corresponding prices
        int[] quantities = new int[items.length]; // array to store ordered quantities
        double totalPrice = 0; // total price accumulator
        boolean continueShopping = true; // flag to loop ordering
        ...
        do {
            ...
            try {
                ...
                if (choice >= 1 && choice <= 7) {
                    ...
                    int index = choice - 1; // adjust index
                    quantities[index] += qty; // add to quantity
